with int_matches as (select 
    concat(t1.team_id, t2.team_id,s.season_id,to_varchar(m.match_date, 'YYYY-MM-DD')) as match_id,
    case
...
from
   {{ ref('stg_matches') }} m join {{ ref('stg_teams') }} t1
    ...
    join {{ ref('stg_formations') }} f1 on f1.formation = m.team_formation_id
    join {{ ref('stg_formations') }} f2 on f2.formation = m.opponent_formation_id
